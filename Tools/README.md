# 🛠 Tool Documentation

Welcome to the **Tool Documentation** section of the **CEH v13 Complete Guide**.

This directory contains **92 documented cybersecurity tools** used throughout the CEH v13 curriculum. Each tool page explains its purpose, key features, practical usage, workflow, advantages, limitations, references, and the CEH module(s) where it is used.

Whether you're studying for the CEH exam or building practical penetration testing skills, this collection serves as a centralized reference for the tools encountered across all 20 CEH modules.

---

# 📊 Collection Overview

| Metric | Value |
|:-------|:-----:|
| 🛠 Tool Documentation | **92** |
| 📚 CEH Modules Covered | **20** |
| 📂 Categories | **14** |
| 🔗 Internal Cross References | ✅ |
| 📖 Practical Usage Examples | ✅ |
| 🧪 Lab Integration | ✅ |

---

# 🗂 Tool Categories

| Category | Description |
|----------|-------------|
| 🌐 Reconnaissance & OSINT | Information gathering and open-source intelligence |
| 🔍 Scanning & Enumeration | Host discovery, service enumeration, and network mapping |
| 🛡 Vulnerability Assessment | Vulnerability scanning and security assessment |
| 🌍 Web Application Security | Web application testing and exploitation |
| 💻 System Hacking | Exploitation, privilege escalation, persistence, and post-exploitation |
| 🔑 Password Attacks | Password auditing and credential attacks |
| 🦠 Malware Analysis & Reverse Engineering | Malware creation, analysis, debugging, and reverse engineering |
| 📡 Network Analysis | Packet capture, sniffing, and traffic monitoring |
| 📶 Wireless Security | Wireless auditing and Wi-Fi security |
| 📱 Mobile Security | Android and mobile penetration testing |
| ☁ Cloud & Infrastructure | Cloud administration and infrastructure utilities |
| 🔐 Cryptography | Encryption, hashing, and cryptographic utilities |
| 🤖 AI-Assisted Security | AI-powered cybersecurity assistants |
| ⚙ System & Administrative Utilities | Windows/Linux administration and forensic utilities |

---

# 🌐 Reconnaissance & OSINT

- [Google Search](Google-Search.md)
- [DNSDumpster](DNSDumpster.md)
- [DomainTools](DomainTools.md)
- [Shodan](Shodan.md)
- [Sherlock](Sherlock.md)
- [Censys](Censys.md)
- [Recon-ng](Recon-ng.md)
- [Netcraft](Netcraft.md)
- [eMailTrackerPro](eMailTrackerPro.md)

---

# 🔍 Scanning & Enumeration

- [Nmap](Nmap.md)
- [Hping3](Hping3.md)
- [RPCScan](RPCScan.md)
- [SuperEnum](SuperEnum.md)
- [SnmpWalk](SnmpWalk.md)
- [nslookup](nslookup.md)
- [Traceroute](Traceroute.md)
- [SmartScanner](SmartScanner.md)
- [Global Network Inventory](Global-Network-Inventory.md)

---

# 🛡 Vulnerability Assessment

- [OpenVAS](OpenVAS.md)
- [Wapiti](Wapiti.md)
- [OWASP ZAP](OWASP-ZAP.md)
- [Searchsploit](Searchsploit.md)
- [Trivy](Trivy.md)

---

# 🌍 Web Application Security

- [Burp Suite](Burp-Suite.md)
- [Caido](Caido.md)
- [Hetty](Hetty.md)
- [sqlmap](sqlmap.md)
- [OWASP ZAP](OWASP-ZAP.md)
- [Wapiti](Wapiti.md)
- [WPScan](WPScan.md)
- [cURL](cURL.md)

---

# 💻 System Hacking & Post-Exploitation

- [Metasploit](Metasploit.md)
- [Netcat](Netcat.md)
- [PowerView](PowerView.md)
- [Responder](Responder.md)
- [Rubeus](Rubeus.md)
- [CrackMapExec](CrackMapExec.md)
- [Impacket](Impacket.md)
- [WinPEAS](WinPEAS.md)
- [Reverse Shell Generator](Reverse-Shell-Generator.md)
- [AADInternals](AADInternals.md)
- [AD Explorer](AD-Explorer.md)

---

# 🔑 Password Attacks

- [Hydra](Hydra.md)
- [Hashcat](Hashcat.md)
- [John the Ripper](John-the-Ripper.md)
- [Cain and Abel](Cain-and-Abel.md)

---

# 🦠 Malware Analysis & Reverse Engineering

- [IDA](IDA.md)
- [OllyDbg](OllyDbg.md)
- [Immunity Debugger](Immunity-Debugger.md)
- [Mona](Mona.md)
- [Detect It Easy (DIE)](Detect-It-Easy.md)
- [Hybrid Analysis](Hybrid-Analysis.md)
- [njRAT](njRAT.md)
- [JPS Virus Maker](JPS-Virus-Maker.md)
- [AndroRAT](AndroRAT.md)

---

# 📡 Network Analysis & Sniffing

- [Wireshark](Wireshark.md)
- [Bettercap](Bettercap.md)
- [TCPView](TCPView.md)
- [CurrPorts](CurrPorts.md)
- [macof](macof.md)
- [Yersinia](Yersinia.md)
- [Snort](Snort.md)

---

# 📶 Wireless Security

- [Aircrack-ng](Aircrack-ng.md)
- [WPScan](WPScan.md)

---

# 📱 Mobile Security

- [PhoneSploit Pro](PhoneSploit-Pro.md)
- [AndroRAT](AndroRAT.md)

---

# ☁ Cloud & Infrastructure

- [AWS CLI](AWS-CLI.md)
- [PuTTY](PuTTY.md)
- [Remmina](Remmina.md)
- [can-utils](can-utils.md)

---

# 🔐 Cryptography

- [CyberChef](CyberChef.md)
- [CryptoForge](CryptoForge.md)
- [VeraCrypt](VeraCrypt.md)
- [Cipher](Cipher.md)

---

# 🤖 AI-Assisted Security

- [ChatGPT](ChatGPT.md)
- [ShellGPT](ShellGPT.md)

---

# ⚙ System & Administrative Utilities

- [BASH](BASH.md)
- [BITSAdmin](BITSAdmin.md)
- [Process Monitor](Process-Monitor.md)
- [Resource Monitor](Resource-Monitor.md)
- [System Monitor](System-Monitor.md)
- [Registry Editor](Registry-Editor.md)
- [Windows Command-Line Utilities](Windows-Command-Line-Utilities.md)
- [wevtutil](wevtutil.md)
- [Telnet](Telnet.md)
- [Refog Personal Monitor](Refog-Personal-Monitor.md)
- [AVG Antivirus](AVG-Antivirus.md)
- [Cowrie](Cowrie.md)
- [CWE](CWE.md)
- [Eagle DDoS](Eagle-DDoS.md)
- [Anti-DDoS Guardian](Anti-DDoS-Guardian.md)
- [UltraDDOS v2](UltraDDOS-v2.md)
- [LOIC (Low Orbit Ion Cannon)](Low-Orbit-Ion-Cannon-LOIC.md)
- [ISB](ISB.md)
- [Social-Engineer Toolkit (SET)](Social-Engineer-Toolkit-SET.md)
- [Spike](Spike.md)

---

# 🎯 Purpose

The goal of this collection is to provide a centralized reference for every cybersecurity tool encountered throughout the CEH v13 learning journey.

Each documentation page is linked from the relevant module, allowing you to understand not only **how each tool works**, but also **when and why it is used** during different phases of an ethical hacking assessment.

---

# 🔗 Related Sections

- [📖 CEH Modules](../Modules/)
- [📚 Cheat Sheets](../Cheat-Sheets/)
- [📂 Resources](../Resources/)
- [🗺 Learning Roadmap](../ROADMAP.md)
- [🏠 Back to Repository](../README.md)